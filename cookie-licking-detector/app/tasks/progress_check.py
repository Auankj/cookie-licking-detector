...
from app.db.models.progress_tracking import ProgressTracking
...
async def _check_claim_progress_async(claim_id: int) -> Dict[str, Any]:
    ...
    async with session_factory() as session:
        try:
            # Get claim and related data with relationships loaded
            ...
            logger.info(f"Checking PRs for issue #{issue.github_issue_number} in {repo.owner_name}/{repo.name}")
            ...
            logger.info(f"Checking commits by {claim.github_username} in {repo.owner_name}/{repo.name}")
            ...
            result = await session.execute(stmt)
            progress_tracking = result.scalar_one_or_none()
            ...
            if user_commits:
                ...
                try:
                    commit_date_str = user_commits[0].get("commit", {}).get("author", {}).get("date", "")
                    if commit_date_str:
                        # Normalize ISO string: replace 'Z' with '+00:00' for Python's fromisoformat
                        if commit_date_str.endswith('Z'):
                            commit_date_str = commit_date_str[:-1] + '+00:00'
                        progress_tracking.last_commit_date = datetime.fromisoformat(commit_date_str)
                    else:
                        progress_tracking.last_commit_date = datetime.now(timezone.utc)
                except (ValueError, TypeError) as e:
                    logger.warning(f"Failed to parse commit date, using current time: {e}")
                    progress_tracking.last_commit_date = datetime.now(timezone.utc)
